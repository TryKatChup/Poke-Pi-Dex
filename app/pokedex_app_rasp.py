...
class App:
    def __init__(self, window, window_title, video_source=0):  # se non specificato viene preso il primo input video
        ...
        self.label_app_version = tk.Label(master=self.frame_menu, text="v" + version, bg=background)
        self.label_app_version.config(font=("Helvetica", 8, "italic"))
        self.label_app_version.pack(side=tk.TOP, pady=(0, 30))
        self.button_start = tk.Button(master=self.frame_menu, text="Start", width=10, bg=background, activebackground=background, command=lambda: self.show_app())
        self.button_start.pack(side=tk.TOP, pady=(0, 10))
        self.button_quit = tk.Button(master=self.frame_menu, text="Quit", width=10, bg=background, activebackground=background, command=lambda: self.quit())
        ...
        self.button_search.pack(side=tk.LEFT, anchor=tk.CENTER)
